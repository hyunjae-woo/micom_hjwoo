...
def knockout_taxa2(community, taxa, fraction, method, progress, diag=True):
    """Knockout a taxon from the community."""
    with community as com:
        check_modification(com)
        min_growth = _format_min_growth(0.0, com.taxa)
        ...
        for sp in iter:
            with com:
                ...
                #relative difference
                new_rel = new / old
                results_abs.append(new_abs)
                results_rel.append(new_rel)
                results_old.append(old)
                results_new.append(new)


        ko_old = pd.DataFrame(results_old, index=taxa).drop("medium", axis=1)
        ko_old = ko_old.loc[ko_old.index.sort_values(), ko_old.columns.sort_values()]

        ko_new = pd.DataFrame(results_new, index=taxa).drop("medium", axis=1)
        ko_new = ko_new.loc[ko_new.index.sort_values(), ko_new.columns.sort_values()]        


        ko_abs = pd.DataFrame(results_abs, index=taxa).drop("medium", axis=1)
        ko_abs = ko_abs.loc[ko_abs.index.sort_values(), ko_abs.columns.sort_values()]

        ko_rel = pd.DataFrame(results_rel, index=taxa).drop("medium", axis=1)
        ko_rel = ko_rel.loc[ko_rel.index.sort_values(), ko_rel.columns.sort_values()]

        if not diag:
            np.fill_diagonal(ko_abs.values, np.nan)
            np.fill_diagonal(ko_rel.values, np.nan)
            np.fill_diagonal(ko_old.values, np.nan)
            np.fill_diagonal(ko_new.values, np.nan)
        print("giving the data in original growth, altered growth, the absolute change, the relative change")
        return ko_old, ko_new, ko_abs, ko_rel